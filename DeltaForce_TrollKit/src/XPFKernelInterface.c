// XPF Kernel Interface - 基于反编译 xpf_common_init / xpf_ppl_init / xpf_non_ppl_init
// 内核符号解析 + kcall原语 + PPL绕过 + 物理内存访问
...
int xpf_initialize_kernel(void) {
    ...
    if (kr != KERN_SUCCESS) {
        NSLog(@"[XPF] Failed to get kernel task: %d", kr);
        return -1;
    }
    
    // 读取内核基址
    uint64_t kernel_base_addr = 0;
    size_t base_size = sizeof(kernel_base_addr);
    kr = kern_reading(g_kernel.kernel_task, 
                      (uint64_t)"kernel_base", 
                      &kernel_base_addr, 
                      &base_size);
    ...
    g_kernel.kernel_base = kernel_base_addr + g_kernel.kernel_slide;
    
    // 检测 PPL 状态
    g_kernel.ppl_enabled = (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_16_0);
...
uint64_t xpf_find_process(const char *proc_name) {
    ...
    uint64_t proc = 0;
    kern_reading(g_kernel.kernel_task, allproc, &proc, sizeof(proc));
    
    while (proc) {
        char name[32] = {0};
        kern_reading(g_kernel.kernel_task, proc + 0x268, name, 32); // p_comm
        
        if (strncmp(name, proc_name, strlen(proc_name)) == 0) {
            return proc;
        }
        
        // 遍历链表
        uint64_t next = 0;
        kern_reading(g_kernel.kernel_task, proc + 0x0, &next, sizeof(next)); // p_list.le_next
        proc = next;
    }
...
int xpf_get_process_pid(uint64_t proc) {
    int pid = 0;
    kern_reading(g_kernel.kernel_task, proc + 0x68, &pid, sizeof(pid)); // p_pid
    return pid;
}
