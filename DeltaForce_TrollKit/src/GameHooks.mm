...
int hooks_patch_recoil(BOOL enable) {
    ...
    if (enable) {
        // 保存原始指令
        kern_reading(g_game_task, recoil_func_addr, &g_original_recoil_code, sizeof(uint32_t));
...
int hooks_patch_no_spread(BOOL enable) {
    ...
    if (spread_func_addr) {
        if (enable) {
            kern_reading(g_game_task, spread_func_addr, &g_original_spread_code, sizeof(uint32_t));
            uint32_t nop = 0x1F2003D5;
...
int hooks_set_all_visible(void) {
    ...
    // 读取本地玩家
    kern_reading(g_game_task, entity_list + 0x10, &local_player, sizeof(uint64_t));
    if (!local_player) return -1;
    
    int local_team = 0;
    kern_reading(g_game_task, local_player + g_game_offsets.team_offset, &local_team, sizeof(int));
    
    // 遍历所有实体
    for (int i = 0; i < 64; i++) {
        uint64_t entity = 0;
        kern_reading(g_game_task, entity_list + i * 8, &entity, sizeof(uint64_t));
        
        if (entity && entity != local_player) {
            int team = 0;
            kern_reading(g_game_task, entity + g_game_offsets.team_offset, &team, sizeof(int));
...
int hooks_aimbot(uint64_t target_entity) {
    ...
    kern_reading(g_game_task, target_entity + g_game_offsets.position_offset, 
                target_pos, sizeof(float) * 3);
    
    // 读取本地玩家位置
    uint64_t local_player = 0;
    kern_reading(g_game_task, g_game_offsets.local_player, &local_player, sizeof(uint64_t));
    if (!local_player) return -1;
    
    float local_pos[3] = {0};
    kern_reading(g_game_task, local_player + g_game_offsets.position_offset, 
                local_pos, sizeof(float) * 3);
...
kern_return_t xpf_attach_kernel_task(uint64_t proc, mach_port_t *task) {
    ...
    kern_return_t kr = kern_reading(g_game_task, proc + 0x10, &task_addr, sizeof(uint64_t));
...
int xpf_inject_dylib(int pid, const char *dylib_path) {
    // 使用内核注入 dylib 到进程
    // 1. 调用 __mac_syscall 或 thread_call
    // 2. 在目标进程中调用 dlopen
