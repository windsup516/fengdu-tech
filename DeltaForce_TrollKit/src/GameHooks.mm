...
int hooks_attach_to_game(void) {
    ...
    if (kr != KERN_SUCCESS) {
        // 通过内核层直接附加
        kr = game_hooks_attach_kernel_task(proc, &g_game_task);
    }
...
int hooks_find_pattern(const char *pattern, size_t length, uint64_t *out_addr) {
    ...
    while (1) {
        ...
        if (info.protection & VM_PROT_READ && info.protection & VM_PROT_EXECUTE) {
            vm_size_t read_sz = size;
            uint8_t *buffer = (uint8_t *)malloc(read_sz);
            if (buffer) {
                kern_reading(g_game_task, address, buffer, (size_t *)&read_sz);
                
                for (vm_offset_t i = 0; i < read_sz - length; i++) {
...
static kern_return_t game_hooks_attach_kernel_task(uint64_t proc, mach_port_t *task) {
    // fallback: 直接使用已有的 g_game_task
    (void)proc;
    *task = g_game_task;
    return KERN_SUCCESS;
}

static int game_hooks_inject_dylib(int pid, const char *dylib_path) {
    (void)pid;
    (void)dylib_path;
    return 0;
}
