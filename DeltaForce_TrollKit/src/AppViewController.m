...
- (void)refreshDeviceInfo {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        while (self.listening) {
            DeviceInfo info = [DeviceInfo shared].currentInfo;
            
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.infoRows.count >= 5) {
                    ((UILabel *)self.infoRows[0]).text = [NSString stringWithFormat:@"FPS: %.0f", info.fps];
                    ((UILabel *)self.infoRows[1]).text = [NSString stringWithFormat:@"Ping: %.0fms", info.ping];
                    ((UILabel *)self.infoRows[2]).text = [NSString stringWithFormat:@"Players: %d", info.playerCount];
                    ((UILabel *)self.infoRows[3]).text = [NSString stringWithFormat:@"Weapon: %@", info.currentWeapon];
                    ((UILabel *)self.infoRows[4]).text = [NSString stringWithFormat:@"State: %@", 
                        info.cheatActive ? @"ACTIVE" : @"IDLE"];
                    
                    // 更新数量计数器
                    self.infoCardCounter.text = [NSString stringWithFormat:@"%d", info.playerCount];
                }
            });
